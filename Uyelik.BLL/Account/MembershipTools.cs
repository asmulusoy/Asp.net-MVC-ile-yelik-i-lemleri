...
namespace Uyelik.BLL.Account
{
    public static class MembershipTools
    {
        ...
        {
            return new UserStore<ApplicationUser>(new MyContext());//Bizim veritabanı mycontext olduğu için onu belirttik tabloları oraya oluşturacak.
        }
        ...
        public static string GetNameSurname()
        {
            ...
            if (string.IsNullOrEmpty(id)) return "Ziyaretçi";
            else
            {
                var userManager = NewUserManager();
                var user = userManager.FindById(id);
                string nameSurname = string.Format("{0} {1}", user.Name, user.Surname);
                return nameSurname;
            }
