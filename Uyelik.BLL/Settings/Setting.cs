    ...
    public static class Setting
    {
        ...
        public async static Task SendMail(MailModel model)
        {
            using (var smtp = new SmtpClient())
            {
                ...
                if (!string.IsNullOrEmpty(model.Bcc)) message.Bcc.Add(new MailAddress(model.Bcc));
                if(!string.IsNullOrEmpty(model.Cc)) message.CC.Add(new MailAddress(model.Cc));
                message.IsBodyHtml = true;
                message.BodyEncoding = Encoding.UTF8;
                var crdential = new NetworkCredential()
                {
                    UserName = SiteMail,
                    Password = SiteMailPassword
                };
                smtp.Credentials = crdential;
